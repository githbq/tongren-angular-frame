...
var source = path.join(__dirname, '../', './build-template');
    ...
    if (options.k) {
        ...
        gulpConfig.forEach(function(line, i) {
            ...
            if (toInsert && (/\s*\}\s*/).test(line)) {
                ...
                var entry = fs.readFileSync(source + '/entry.txt', 'utf8');
        ...
        routeConfig.forEach(function(line, i) {
            ...
            if (page.length === 1) {
                if (toInsert && (/\s*\}\s*/).test(line) && !next) {
                    ...
                    var hash = fs.readFileSync(source + '/hash.txt');
                    hash = hash.toString();
                ...
                if (srcConfig.pageConfig[page[0]].subPage) {
                    ...
                    if (toInsert && beLine && hasReg.test(beLine)) {
                        var subhash = fs.readFileSync(source + '/subhash.txt');
                    ...
                    if (toInsert && hasReg.test(beLine)) {
                        ...
                        var subhashfirst = fs.readFileSync(source + '/subhashfirst.txt');
        ...
        var controller = fs.readFileSync(source + '/controller.txt');
        ...
        var router = fs.readFileSync(source + '/router.txt');
...
gulp.task('rm', function() {
    if (options.k) {
        var page = options.k.split('_').slice(0, 2);
        // 文件名是否合法
        if (!(/^[a-z0-9]+_*[a-z0-9]+$/).test(options.k)) {
            gutil.log(chalk.red('the page name is made of a-z0-9'));
            return false;
        }
...
gulp.task('rename', function() {
    if (options.k) {
        var page = options.k.split('_').slice(0, 2);
        // 文件名是否合法
        if (!(/^[a-z0-9]+_*[a-z0-9]+2[a-z0-9]+_*[a-z0-9]+$/).test(options.k)) {
            gutil.log(chalk.red('the page name is made of a-z0-9'));
            return false;
        }
    }
